...
public class AdminKeyData {
    ...
    public ArrayList<AdminKey> GetAllAdminKeys() {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM adminkeys");
    ...
    public Boolean CheckSuperadmin(String key) {
        try {
            connection = OpenConnection();
            Statement stmt = connection.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM superadminkeys WHERE adminkey = '"+ key +"';");
            if(rs.next()) {
                stmt.close();
                connection.close();
                return true;
    ...
    public void CreateAdminkey(AdminKey adminKey) {
        try {
            ...
            int status = stmt.executeUpdate("INSERT INTO adminkeys (`adminkey`,`ssid`) VALUES ('"+ adminKey.getKey()  +"', '" + adminKey.getSSID() + "');");
    ...
    public void UpdateAdminKey(AdminKey adminKey, String oldAdminKey) {
        try {
            ...
            int status = stmt.executeUpdate("UPDATE adminkeys SET adminkey = '"+adminKey.getKey()+"', ssid = '"+ adminKey.getSSID() +"' WHERE adminkey = '"+ oldAdminKey +"'");
    ...
    public void DeleteAdminKey(String key) {
        try {
            ...
            status += String.valueOf(stmt.executeUpdate("DELETE FROM `adminkeys` WHERE adminkey='"+ key +"';"));
